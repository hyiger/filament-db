...
- **Bambu Studio** -- import a Bambu Studio `.json` filament preset directly (single-filament sync button on the filament detail page, or bulk import via the `/import-export` tile). Preserves variant inheritance + auto-detects calibration context from the preset's `printer_settings_id`. Round-trip safe: export → re-import is lossless even for slicer keys the app doesn't model. OrcaSlicer is **export-only** today (`GET /api/filaments/orcaslicer` bundle, single-filament `GET /api/filaments/{id}/orcaslicer`); bulk Orca import is stubbed at 501 pending a future PR
