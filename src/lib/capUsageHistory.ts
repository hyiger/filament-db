/**
 * Undo-aware cap on a spool's embedded `usageHistory` array (GH #954).
 *
 * The cap stops a client looping usage POSTs from growing a filament document
 * toward MongoDB's 16 MiB limit (GH #304). The obvious `entries.slice(-max)`
 * evicts the OLDEST entries — exactly the set most likely to include a
 * still-live `source:"job"`/`"slicer"` entry, and `DELETE
 * /api/print-history/{id}` refunds spool weight ONLY when it finds and removes
 * the job's matching `usageHistory` entry (the GH #621 idempotency guard:
 * "entry gone ⇒ already refunded ⇒ skip"). Evicting a live job's entry makes
 * its later DELETE silently skip the refund → a permanent inventory undercount.
 *
 * So this trim preferentially evicts entries NO undo path needs — manual/nfc
 * logs (jobId null, source `manual`/`nfc`) — oldest-first. Undo-relevant
 * entries are evicted only as a last resort, when the array is ENTIRELY
 * undo-relevant (a pathological 1000-live-jobs-on-one-spool case). Append
 * order is preserved throughout.
 *
 * CONTRACT: callers push-then-cap, so the LAST entry is the row just recorded —
 * it is never evicted. Without that guard, a spool already full of
 * undo-relevant entries would drop the freshly appended manual/nfc row while
 * the caller has already debited weight and returned 201 — a silent loss of
 * the just-logged use. Protecting the newest row means the last-resort
 * eviction can sacrifice an OLD undo entry to keep it; losing what the user
 ...
 * predate that cap — or arrive through paths that bypass the routes (snapshot
 * restore, hybrid sync) — can still carry a pathological magnitude.
 * Sanitising at INGESTION rather than at each emit point keeps every
 * downstream invariant intact by construction (see MAX_USAGE_GRAMS for the
 * Infinity → NaN → JSON null cascade). A clamped row renders AT the cap
 * rather than vanishing, so the corruption stays visible instead of silently
 * under-reporting; a NaN/negative/missing value contributes 0.
 *
 * GH #1078: shared with the dashboard route so
 * `recentPrintHistory[].totalGrams` clamps through the SAME sanitizer.
 */
