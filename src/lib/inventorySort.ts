...
/**
 * Net remaining filament grams: gross `totalWeight` minus the empty-spool tare
 * (variant's own `spoolWeight`, else parent's, else a 0g fallback). The 0g
 * fallback matches `/api/spools/by-location`'s own totalGrams math and the
 * page's filtered-group recompute (Codex P2 round 4 on PR #400). `null` when no
 * `totalWeight` — those sort/total as "unknown".
 */
...
function sortValue(row: InventoryRow, key: InventorySortKey): number | string | null {
  switch (key) {
    case "remaining":
      return inventoryRemainingGrams(row);
    case "name":
