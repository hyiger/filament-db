...
export async function GET() {
  ...
  for (const filament of filaments) {
    ...
    if (calibrations && calibrations.length > 0) {
      // Generate one section per calibration with overrides merged
      for (const cal of calibrations) {
        if (!cal.nozzle) continue;
        const nozzleSuffix = cal.nozzle.name;
        const sectionName = `${filament.name} ${nozzleSuffix}`;
        ...
        // Pressure advance goes into start_filament_gcode M572
      ...
      // No calibrations — export as single section
      writeSection(lines, filament.name, settings);
    }
  }
