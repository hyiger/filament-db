...
adopted by each sibling that doesn't override it. Five known cases, all needing the same
post-create move:

| Stranded | Consequence for later siblings | How to move it |
|---|---|---|
| `settings` (esp. `filament_abrasive`) | every colour inherits a pigment-driven flag; the slicer and a firmware `M862.1` check then treat an ordinary colour as abrasive | shallow-MERGED by the resolver, so no variant array is needed to reach siblings — `PUT` a pigment-driven key onto the variant and clear it from the template |
