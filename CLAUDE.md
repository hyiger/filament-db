...
- **Encoder is pure JS, browser-safe** (`src/lib/labelEncoder.ts`). Implements Brother's [Raster Command Reference for PT-E550W/P750W/P710BT](https://download.brother.com/welcome/docp100064/cv_pte550wp750wp710bt_eng_raster_102.pdf). Shared by the renderer dialog, the standalone CLI spike (`scripts/print-label-spike.ts`), and the simulator that decodes the byte stream back into a PNG (`scripts/print-label-sim.ts`). Tape width 24mm rounds to media-byte 24; 3.5mm rounds to 4 (Brother's wire convention — `Math.round` covers both).
