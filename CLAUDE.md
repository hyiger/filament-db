...
## Brother PT-P710BT label printer (unmerged on `feat/label-printer`)

End-to-end Bluetooth label printing for the Brother PT-P710BT (P-touch CUBE) — Electron-only feature, currently on the `feat/label-printer` branch (PR #487, not yet merged). Architecture worth knowing about before touching the code:

- **Transport is Bluetooth Classic SPP/RFCOMM, not BLE.** Web Bluetooth in Chromium only supports BLE GATT, so the renderer can't talk to the printer directly. The pragmatic path: let the OS handle pairing (System Settings → Bluetooth), get back a serial-port path (`/dev/tty.PT-P710BT-XXXX-Serialport` on macOS, `COMx` on Windows, `/dev/rfcomm0` on Linux), then use `serialport` npm from the Electron main process to write the byte stream. No native BT addons.
...
- **Bitmap rendering** (`src/lib/labelBitmap.ts`) is browser-only — uses HTMLCanvasElement + the `qrcode` npm. Renders QR + filament name at the print head's native 128-dot density, rotates 90° for raster-line orientation, thresholds to 1-bit grayscale ready for `packGrayscaleBitmap()`. QR sizing picks the largest module scale that fits `PRINT_HEAD_DOTS - 2 * VERTICAL_PADDING_DOTS = 116` dots after including the spec-required 4-module quiet zone (rendered with `margin: 4`). Payloads too long to fit even at scale 1 throw — no silent clipping.
- **Transport** (`electron/label-printer.ts`) wraps `serialport` with a 25s stall watchdog armed BEFORE `port.open()` (covers a hung open phase, not just hung writes). Every failure path — construct error, open error, write error, drain error, async `error` event, watchdog stall — routes through one `settleWithCleanup(err)` that closes the port if open BEFORE rejecting. Belt-and-braces in the open callback: if `settled` is already true when open() finally returns (timer fired during a hung open), best-effort close any handle that did get acquired so the next print doesn't see "port busy".
...
- **Packaged build**: `electron-builder.yml` includes `node_modules/serialport/**/*`, `node_modules/@serialport/**/*`, AND `node_modules/node-gyp-build/**/*` (the runtime loader for the .node binary), `node-addon-api/**/*`, `debug/**/*`, `ms/**/*`. Without the runtime loader the packaged app crashes at first label-printer interaction with "Cannot find module 'node-gyp-build'". `npm run electron:rebuild` includes `-o @serialport/bindings-cpp` alongside the existing pcsclite rebuild.
- **Settings UI**: `src/components/LabelPrinterSettings.tsx` renders only in Electron (`useIsElectron`), mounted from `src/app/settings/page.tsx`. Lists every serial port and badges PT-Touch matches. Public URL field validates through the IPC handler and surfaces the main-process error inline.
...
- **Scan-back resolution**: the instance-ID QR mode encodes the bare 5-byte hex; `GET /api/filaments/match?instanceId=<hex>` resolves it (added late in PR #487's iteration). Priority order in the match endpoint: `instanceId` → `name` → `vendor+type` → `vendor` alone. Exact-case match preferred; case-insensitive fallback only when unique (a `ABC`/`abc` legacy collision returns both as `candidates` instead of an arbitrary pick). The instanceId field accepts any string up to 128 chars — the schema doesn't enforce hex, so legacy/imported IDs like `custom-id-123` still resolve. `escapeRegex` makes the case-insensitive comparison literal so regex-special chars in stored IDs (or in the query) don't cause false matches.
...
**Caveats / known constraints** (carried into Tuesday's real-printer test):
- Brother's official line is "PT-P710BT Bluetooth is iOS/Android only" — the desktop SPP path is community-validated, not vendor-blessed. Pin a known-working firmware version in support docs if it becomes a habit.
- macOS 15+ Bluetooth-backed serial-port TCC prompt fires the first time on a new install.
- Windows 11 outgoing COM port assignment can be flaky; document "re-pair if it disappears" as a recovery path.
