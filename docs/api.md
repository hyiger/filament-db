...
The exported bundle stamps every preset with a `filamentdb_id` config key (the filament's stable `_id`); the sync consumes it for routing only — it is never stored in the `settings` bag. Per-nozzle suffixed presets from a multi-nozzle export (#876) additionally carry a `filamentdb_nozzle` hint, which routes their baked calibration keys to the matching per-nozzle calibration entry (and stops the suffixed name being read as a rename).
