...
A **threshold-only** parent is the other ungated case, on every one of these routes. `lowStockThreshold` deliberately doesn't count as carrying — a threshold with no inventory behind it moves nothing worth confirming — so a parent that stores one while having no `color`, no `colorName`, no spools and no `totalWeight` mints its first variant with no `409`. The threshold is dead config the moment that variant exists (the low-stock alarm would otherwise evaluate against a template), so it is cleared on the parent *after* the variant is written, and the clear is reported in no response field. It is **not** copied onto the new variant: a client that wants to keep the alarm must send `lowStockThreshold` on the variant itself. Neither a *carrying* parent nor one that already has variants is touched — the first moves its threshold with the inventory during the promotion described above, the second keeps its value under enforce-forward.
...
Each emitted section also includes `compatible_printers = ` and `compatible_printers_condition = ` (both empty) by default, which PrusaSlicer treats as "no restriction" — the synced filament shows up in every printer's dropdown, and the scan-stream auto-select works regardless of which printer profile is active. If a user pinned a specific restriction via a previous round-trip import (the keys arrive non-empty in the settings bag), the export preserves that restriction. Both keys are visible and editable on the filament form's Slicer tab (#1066) — a preset duplicated in the slicer under another printer's profile carries its source's restriction (e.g. `printer_model=~/(COREONE)/`), which otherwise hides the synced preset on every other printer; clearing the field writes an explicit empty value, making the preset visible everywhere again. Note the slicer sync echoes the whole preset back, so a restriction cleared in-app is re-applied by the next sync from a still-restricted preset in a running PrusaSlicer — refresh the slicer-side preset (restart or re-import) after clearing.

The sync (`POST /api/filaments/:id`) also lifts `inherits` out of the settings bag onto the top-level field (#1066), matching the bulk INI import: pre-#1066 syncs re-created a bag copy that kept exporting even after the top-level field was cleared. A stored bag copy is purged on the next sync (its value is adopted top-level first when the effective — parent-resolved — value was empty; a parent-supplied `inherits` masked the copy in every export, so it is purged without adoption there).
...
Snapshot schema version is `6`. The history: v2 added bed types, v3 locations + print history, v4 shared catalogs (v1.14.0), v5 the top-level `legacyNozzleCleanupComplete` provenance flag, and v6 `Location.desiccantChangedAt`. Older snapshots still restore — collections a v1/v2/v3 file doesn't carry are **left untouched** and reported in a `skipped` array on the restore response (GH #1104); a key present as an explicit **empty array** still empties that collection. The v5 and v6 bumps exist specifically so an **older** build refuses the file (see the restore guard below) rather than accepting it and silently dropping the field its schema doesn't know about.
