...
A filament with zero or one nozzle calibration is exported as a single `[filament:Name]` section; its calibration values (EM, pressure advance, max volumetric speed, retraction) are not baked into the INI — they are applied dynamically at print time via the `/api/filaments/{id}/calibration` endpoint (used by PrusaSlicer Filament Edition). A filament calibrated for **two or more distinct nozzles** exports one section per nozzle instead, name-suffixed with the nozzle (e.g. `[filament:PLA 0.4 Brass]`) with that nozzle's filament-scoped calibration values baked in (pressure advance stays dynamic via the calibration API) — so "extra" suffixed sections in the bundle are expected, not duplicates. If a filament is missing from the export entirely, check that it has a name and is not soft-deleted. **Templates are deliberately left out** (since v1.80): a filament that has color variants is a template, not printable stock, so it never becomes a slicer preset — export one of its variants instead (each variant carries the template's inherited settings).
