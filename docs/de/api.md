...
Jeder ausgegebene Abschnitt enthält außerdem standardmäßig `compatible_printers = ` und `compatible_printers_condition = ` (beide leer), was PrusaSlicer als „keine Einschränkung" interpretiert — das synchronisierte Filament erscheint im Dropdown jedes Druckers, und die Auto-Auswahl des Scan-Streams funktioniert unabhängig davon, welches Druckerprofil aktiv ist. Hat ein Nutzer über einen vorherigen Round-Trip-Import eine spezifische Einschränkung gesetzt (die Schlüssel kommen non-empty im Settings-Bag an), bleibt diese Einschränkung beim Export erhalten. Beide Schlüssel sind auf dem Slicer-Tab des Filamentformulars sichtbar und bearbeitbar (#1066) — eine im Slicer unter dem Profil eines anderen Druckers duplizierte Voreinstellung trägt die Einschränkung ihrer Quelle mit (z. B. `printer_model=~/(COREONE)/`), die die synchronisierte Voreinstellung sonst auf jedem anderen Drucker verbirgt; das Leeren des Feldes schreibt einen expliziten Leerwert, womit die Voreinstellung wieder überall sichtbar ist. Beachten Sie: Die Slicer-Synchronisation schickt die gesamte Voreinstellung zurück, eine in der App geleerte Einschränkung wird also von der nächsten Synchronisation einer noch eingeschränkten Voreinstellung in einem laufenden PrusaSlicer erneut gesetzt — aktualisieren Sie die Voreinstellung auf Slicer-Seite (Neustart oder Re-Import) nach dem Leeren.

Die Synchronisation (`POST /api/filaments/:id`) hebt außerdem `inherits` aus dem Settings-Bag auf das Top-Level-Feld (#1066), wie es der Bulk-INI-Import tut: Synchronisationen vor #1066 erzeugten eine Bag-Kopie neu, die auch nach dem Leeren des Top-Level-Feldes weiter exportiert wurde. Eine gespeicherte Bag-Kopie wird bei der nächsten Synchronisation entfernt (ihr Wert wird zuerst auf das Top-Level-Feld übernommen, wenn der effektive — über das Elternfilament aufgelöste — Wert leer war; ein vom Elternfilament geliefertes `inherits` hat die Kopie in jedem Export verdeckt, dort wird sie ohne Übernahme entfernt).
...
Die Snapshot-Schema-Version ist `7`. Die Historie: v2 ergänzte die Druckbett-Typen, v3 Locations + Druckverlauf, v4 die Shared Catalogs (v1.14.0), v5 das Top-Level-Provenance-Flag `legacyNozzleCleanupComplete`, v6 `Location.desiccantChangedAt` und v7 `debitedGrams` in beiden Nutzungs-Ledgern (GH #1074). Ältere Snapshots lassen sich weiterhin wiederherstellen — Collections, die eine v1-/v2-/v3-Datei nicht mitführt, bleiben **unangetastet** und werden in einem `skipped`-Array der Restore-Antwort benannt (GH #1104); ein Schlüssel, der als explizit **leeres Array** vorhanden ist, leert diese Collection weiterhin. Die Sprünge auf v5, v6 und v7 existieren gerade deshalb, damit ein **älterer** Build die Datei ablehnt (siehe die Restore-Sperre weiter unten), statt sie anzunehmen und stillschweigend das Feld zu verwerfen, das sein Schema nicht kennt — bei v7 ist das genau jenes Feld, das eine Über-Gutschrift bei gekappter Abbuchung verhindert.
