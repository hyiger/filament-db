...
> ⚠️ **Bekannte Einschränkung (macOS-Auto-Update):** macOS wird von zwei separaten, nicht geordneten Jobs gebaut (arm64 und x64), die jeweils ein eigenes `latest-mac.yml` ohne Architektur-Suffix erzeugen. Beim Upload überschreibt das letzte das andere, sodass nur ein `latest-mac.yml` im Release verbleibt — **welche Architektur es referenziert, ist nicht deterministisch** (je nachdem, welcher mac-Job zuletzt hochlädt; im v1.34.4-Release war es der arm64-Build). Die *jeweils andere* macOS-Architektur kann sich dann nicht über das Manifest automatisch aktualisieren. (Als Release-Workflow-Korrektur vorgemerkt.)
