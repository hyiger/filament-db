...
> **Das Löschen eines NTAG ermittelt die Chip-Größe automatisch — auch auf Readern, die die Größenabfrage verweigern.** Vor dem Löschen fragt die App den Chip nach seiner Größe (`GET_VERSION`); verweigert ein Reader diese Abfrage (der ACR1552U antwortet mit `SW 0x6900`), tastet die App die Kapazität stattdessen direkt mit harmlosen Lesezugriffen ab (216 → 215 → 213) und löscht in der erkannten Größe — ohne Nachfrage, ohne Fehler (#978). Ein von einem früheren Schreibvorgang falsch dimensionierter Tag wird durch das Löschen auf seine wahre Größe zurückgesetzt. Das Löschen von OpenPrintTag-Tags (SLIX2) ist davon nicht betroffen.
