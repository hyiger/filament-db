...
> **Das Löschen eines NTAG erfordert eine belegte Größe.** Vor dem Löschen fragt die App den Chip nach seiner Größe (`GET_VERSION`). Verweigert ein Reader diese Abfrage (der ACR1552U antwortet mit `SW 0x6900`), belegt die App einen vollständigen NTAG216 mit harmlosen Lesezugriffen und löscht ihn ohne Nachfrage; ein kleinerer oder passwortgeschützter Tag wird mit Begründung abgelehnt — beides ist auf einem solchen Reader nicht unterscheidbar, und ein Löschen auf Verdacht könnte alte Daten zurücklassen und trotzdem Erfolg melden (#978). **NFC schreiben** funktioniert dort weiterhin für jede Größe (es setzt die Größe explizit und schreibt den Capability Container neu, was auch falsch dimensionierte Tags korrigiert). Das Löschen von OpenPrintTag-Tags (SLIX2) ist davon nicht betroffen.
